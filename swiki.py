...
def setupArgs():
    ...
    argparser = argparse.ArgumentParser(
        description="Create wiki at output dir from input dir."
    )
    argparser.add_argument(
        "input_dir", metavar="input", type=str, help="the path to the input directory"
    )
    argparser.add_argument(
        "output_dir",
        metavar="output",
...
def copy_css_file(pages_dir: str, output_dir: str):
    ...
    if os.path.isdir(swiki_folder := os.path.join(pages_dir, "_swiki")):
        ...
        shutil.copy2(
            os.path.join(swiki_folder, css_file), os.path.join(output_dir, css_file)
        )
...
def make_page_dict(
    subfolder: str, file: str, rel_path: str, isIndex: bool = False
) -> dict:
...
def make_fatfile(info: dict, fatfile: str, frame: str, output_dir: str):
    ...
    fatfile = (
        "<h1>Fatfile</h1><p>This file contains the contents of every page in the wiki in no order whatsoever.</p>"
        + fatfile
    )
...
def make_sitemap(index: dict, sitemap: dict, frame: str, output_dir: str):
    ...
    def convert_folder_to_html(folder: str, display_name: str = None) -> str:
        ...
        html = ""
        sorted_folder_list = sorted(
            sitemap.get(folder), key=lambda page: page.get("title").lower()
        )
...
def make_wiki(pages_dir: str, output_dir: str):
        ...
        for file in files:
            ...
            for link in page["links"]:
                ...
                pages[link_filename]["backlinks"].append(
                    {
                        "title": page["metadata"].get("title", page_filename),
    ...
    for page, info in pages.items():
        ...
        if not info.get("metadata"):
            info["metadata"] = dict()
        info["metadata"] = {
            "title": info["metadata"].get("title", page),
        ...
        if args.build_fatfile:
            fatfile_content = re.sub(
                rf'(?<=<h1 id="title">){info["metadata"].get("title")}(?=</h1>)',
        ...
        sitemap = add_page_to_sitemap(
            {"title": info["metadata"].get("title"), "filename": page},
    ...
    if args.build_fatfile:
        make_fatfile(
            {"metadata": {"title": f'{index["metadata"].get("title")} - Fatfile'}},
            fatfile,
            frame,
            output_dir,
        )
